...
public class UserController {
    ...
    public Result login(@RequestBody LoginForm form){
        ...
        return new Result(Constants.SUCCESS, dto, "Login success");
    }

    @PostMapping("/register")
    public Result register(@RequestBody LoginForm form){
        UserDTO dto = userService.register(form);
        return new Result(Constants.SUCCESS, dto, "Register success");
    }

    @GetMapping("/userinfo/{id}")
    public Result getUserInfo(@PathVariable int id){
        User user = userService.getUserInfo(id);
        return new Result(Constants.SUCCESS, user, "UserInfo by id");
    }

    @GetMapping("/user")
    public Result getCurrentUser(){
        User currentUser = userService.getCurrentUser();
        return new Result(Constants.SUCCESS, currentUser, "Login User currently");
    }
